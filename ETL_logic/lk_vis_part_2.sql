...
-- -------------------------------------------------------------------
DROP TABLE IF EXISTS etl_dataset.lk_visit_no_hadm_all;
CREATE TABLE etl_dataset.lk_visit_no_hadm_all AS
-- labevents
SELECT
...
-- -------------------------------------------------------------------
DROP TABLE IF EXISTS etl_dataset.lk_visit_no_hadm_dist;
CREATE TABLE etl_dataset.lk_visit_no_hadm_dist AS
SELECT
    src.subject_id                                  AS subject_id,
...
FROM
    etl_dataset.lk_visit_no_hadm_all src
GROUP BY
    src.subject_id,
    src.date_id
;
...
--      lk_meas_waveform_mapped
-- -------------------------------------------------------------------
DROP TABLE IF EXISTS etl_dataset.lk_visit_detail_waveform_dist;
CREATE TABLE etl_dataset.lk_visit_detail_waveform_dist AS
SELECT
    src.subject_id                                  AS subject_id,
    ...
    0                               AS load_row_id,
    jsonb_build_object('subject_id', src.subject_id, 'hadm_id', src.hadm_id, 'reference_id', src.reference_id )                                 AS trace_id
FROM
    etl_dataset.lk_meas_waveform_mapped src
GROUP BY
    src.subject_id,
...
-- lk_visit_clean
-- -------------------------------------------------------------------
DROP TABLE IF EXISTS etl_dataset.lk_visit_clean;
CREATE TABLE etl_dataset.lk_visit_clean AS
SELECT
    ...
    src.trace_id                    AS trace_id
FROM
    etl_dataset.lk_admissions_clean src -- adm
UNION ALL
SELECT
    ('x'||substr(md5(random():: text),1,8))::bit(32)::int               AS visit_occurrence_id,
    src.subject_id                                  AS subject_id,
    ...
    src.trace_id                    AS trace_id
FROM
    etl_dataset.lk_visit_no_hadm_dist src -- adm
;
...
-- -------------------------------------------------------------------
DROP TABLE IF EXISTS etl_dataset.lk_visit_detail_clean;
CREATE TABLE etl_dataset.lk_visit_detail_clean AS
SELECT
    ...
    src.trace_id                    AS trace_id
FROM 
    etl_dataset.lk_transfers_clean src
WHERE
...
-- ER admissions
-- -------------------------------------------------------------------
INSERT INTO etl_dataset.lk_visit_detail_clean
SELECT
    ('x'||substr(md5(random():: text),1,8))::bit(32)::int               AS visit_detail_id,
    src.subject_id                                  AS subject_id,
    src.hadm_id                                     AS hadm_id,
    ...
    src.trace_id                    AS trace_id
FROM 
    etl_dataset.lk_admissions_clean src
WHERE
    src.is_er_admission
;
...
SELECT
    ...
    src.trace_id                    AS trace_id
FROM 
    etl_dataset.lk_services_clean src
WHERE
...
SELECT
    ...
    src.trace_id                    AS trace_id
FROM 
    etl_dataset.lk_visit_detail_waveform_dist src
;
...
-- skip "mapped"
-- -------------------------------------------------------------------
DROP TABLE IF EXISTS etl_dataset.lk_visit_detail_prev_next;
CREATE TABLE etl_dataset.lk_visit_detail_prev_next AS
SELECT 
    ...
    src.trace_id                      AS trace_id
FROM 
    etl_dataset.lk_visit_detail_clean src
LEFT JOIN 
    etl_dataset.lk_visit_clean vis
...
-- -------------------------------------------------------------------
DROP TABLE IF EXISTS etl_dataset.lk_visit_concept;
CREATE TABLE etl_dataset.lk_visit_concept AS
SELECT 
