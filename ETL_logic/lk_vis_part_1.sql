...
-- -------------------------------------------------------------------
DROP TABLE IF EXISTS etl_dataset.lk_admissions_clean;
CREATE TABLE etl_dataset.lk_admissions_clean AS
SELECT
...
-- -------------------------------------------------------------------
DROP TABLE IF EXISTS etl_dataset.lk_transfers_clean;
CREATE TABLE etl_dataset.lk_transfers_clean AS
SELECT
    src.subject_id                                  AS subject_id,
...
LEFT JOIN
   		etl_dataset.lk_admissions_clean vis -- associate transfers with admissions according to 
...
-- -------------------------------------------------------------------
DROP TABLE IF EXISTS etl_dataset.lk_services_duplicated;
CREATE TABLE etl_dataset.lk_services_duplicated AS
SELECT
    trace_id, COUNT(*) AS row_count
FROM 
    etl_dataset.src_services src
GROUP BY
    src.trace_id
HAVING COUNT(*) > 1
;

DROP TABLE IF EXISTS etl_dataset.lk_services_clean;
CREATE TABLE etl_dataset.lk_services_clean AS
SELECT
    src.subject_id                                  AS subject_id,
    src.hadm_id                                     AS hadm_id,
...
FROM 
   etl_dataset.src_services src
LEFT JOIN
   etl_dataset.lk_services_duplicated sd
