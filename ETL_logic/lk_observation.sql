...
-- rules 1-3
-- -------------------------------------------------------------------
DROP TABLE IF EXISTS etl_dataset.lk_observation_clean;
CREATE TABLE etl_dataset.lk_observation_clean AS
-- rule 1, insurance
SELECT
...
INSERT INTO etl_dataset.lk_observation_clean
SELECT
    src.subject_id                  AS subject_id,
    src.hadm_id                     AS hadm_id,
    ...
    src.trace_id                    AS trace_id
FROM
    etl_dataset.src_drgcodes src -- drg
INNER JOIN
...
-- Rules 1-4
-- -------------------------------------------------------------------
DROP TABLE IF EXISTS etl_dataset.lk_obs_admissions_concept;
CREATE TABLE etl_dataset.lk_obs_admissions_concept AS
SELECT DISTINCT
...
FROM
    etl_dataset.lk_observation_clean src
LEFT JOIN
...
-- -------------------------------------------------------------------
DROP TABLE IF EXISTS etl_dataset.lk_observation_mapped;
CREATE TABLE etl_dataset.lk_observation_mapped AS
SELECT
...
FROM
    etl_dataset.lk_observation_clean src
LEFT JOIN
    etl_dataset.lk_obs_admissions_concept lc
