...
-- Rule 1
-- -------------------------------------------------------------------
DROP TABLE IF EXISTS etl_dataset.lk_prescriptions_clean;
CREATE TABLE etl_dataset.lk_prescriptions_clean AS
SELECT
    ...
    CAST((REGEXP_MATCHES(src.form_val_disp, r'[-]?[\d]+[.]?[\d]*'))[1] AS FLOAT)  AS quantity,
    -- COALESCE(
...
FROM
    etl_dataset.src_prescriptions src -- pr
LEFT JOIN 
    etl_dataset.src_pharmacy pharm
...
-- -------------------------------------------------------------------
DROP TABLE IF EXISTS etl_dataset.lk_pr_ndc_concept;
CREATE TABLE etl_dataset.lk_pr_ndc_concept AS
SELECT DISTINCT
...
FROM
    etl_dataset.lk_prescriptions_clean src -- pr
LEFT JOIN
...
-- Rule 1
-- -------------------------------------------------------------------
DROP TABLE IF EXISTS etl_dataset.lk_pr_gcpt_concept;
CREATE TABLE etl_dataset.lk_pr_gcpt_concept AS
SELECT DISTINCT
...
-- Rule 1
-- -------------------------------------------------------------------
DROP TABLE IF EXISTS etl_dataset.lk_pr_route_concept;
CREATE TABLE etl_dataset.lk_pr_route_concept AS
SELECT DISTINCT
...
-- lk_drug_mapped
-- -------------------------------------------------------------------
DROP TABLE IF EXISTS etl_dataset.lk_drug_mapped;
CREATE TABLE etl_dataset.lk_drug_mapped AS
SELECT
    src.hadm_id                                     AS hadm_id,
    src.subject_id                                  AS subject_id,
