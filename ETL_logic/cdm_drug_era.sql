DROP TABLE IF EXISTS etl_dataset.lk_join_voc_drug;
CREATE TABLE etl_dataset.lk_join_voc_drug
AS SELECT DISTINCT
...
-- -------------------------------------------------------------------
DROP TABLE IF EXISTS etl_dataset.tmp_pretarget_drug;
CREATE TABLE etl_dataset.tmp_pretarget_drug
AS SELECT
...
FROM
    etl_dataset.cdm_drug_exposure d
JOIN
    etl_dataset.lk_join_voc_drug v
        ON v.descendant_concept_id = d.drug_concept_id
WHERE
    d.drug_concept_id != 0
;
DROP TABLE IF EXISTS etl_dataset.tmp_subenddates_un_drug;
CREATE TABLE etl_dataset.tmp_subenddates_un_drug
    AS SELECT
    ...
    FROM
        etl_dataset.tmp_pretarget_drug
UNION ALL
    SELECT
        person_id                   AS person_id,
        ingredient_concept_id       AS ingredient_concept_id,
    ...
    FROM
        etl_dataset.tmp_pretarget_drug
;
DROP TABLE IF EXISTS etl_dataset.tmp_subenddates_rows_drug;
CREATE TABLE etl_dataset.tmp_subenddates_rows_drug
AS SELECT
    person_id                       AS person_id,
    ingredient_concept_id           AS ingredient_concept_id,
...
FROM
    etl_dataset.tmp_subenddates_un_drug
;
DROP TABLE IF EXISTS etl_dataset.tmp_subenddates_drug;
CREATE TABLE etl_dataset.tmp_subenddates_drug
AS SELECT
    person_id               AS person_id,
    ingredient_concept_id   AS ingredient_concept_id,
    event_date              AS end_date
FROM
    etl_dataset.tmp_subenddates_rows_drug e
WHERE
    (2 * e.start_ordinal) - e.overall_ord = 0
;
DROP TABLE IF EXISTS etl_dataset.temp_ends_drug;
CREATE TABLE etl_dataset.temp_ends_drug
AS SELECT
...
FROM
    etl_dataset.tmp_pretarget_drug dt
JOIN
    etl_dataset.tmp_subenddates_drug e
...
;
DROP TABLE IF EXISTS etl_dataset.tmp_sub_drug;
CREATE TABLE etl_dataset.tmp_sub_drug
AS SELECT
    ROW_NUMBER() OVER (
        PARTITION BY
            person_id,
...
ORDER BY
    ...
    drug_concept_id
;
DROP TABLE IF EXISTS etl_dataset.tmp_finaltarget_drug;
CREATE TABLE etl_dataset.tmp_finaltarget_drug
AS SELECT
...
FROM
    etl_dataset.tmp_sub_drug
;
DROP TABLE IF EXISTS etl_dataset.tmp_enddates_un_drug;
CREATE TABLE etl_dataset.tmp_enddates_un_drug
    AS SELECT
        person_id                                       AS person_id,
        ingredient_concept_id                           AS ingredient_concept_id,
    ...
    FROM
        etl_dataset.tmp_finaltarget_drug
UNION ALL
-- pad the end dates by 30 to allow a grace period for overlapping ranges.
    ...
    FROM
        etl_dataset.tmp_finaltarget_drug
;
DROP TABLE IF EXISTS etl_dataset.tmp_enddates_rows_drug;
CREATE TABLE etl_dataset.tmp_enddates_rows_drug
AS SELECT
    person_id                       AS person_id,
    ingredient_concept_id           AS ingredient_concept_id,
    event_date                      AS event_date,
    ...
    ROW_NUMBER() OVER (
      ...
      -- this re-numbers the inner UNION so all rows are numbered ordered by the event date
FROM
    etl_dataset.tmp_enddates_un_drug
;
DROP TABLE IF EXISTS etl_dataset.tmp_enddates_drug;
CREATE TABLE etl_dataset.tmp_enddates_drug
AS SELECT
    person_id                                       AS person_id,
    ingredient_concept_id                           AS ingredient_concept_id,
    DATE(event_date - INTERVAL '30 DAY')          AS end_date  -- unpad the end date
FROM
    etl_dataset.tmp_enddates_rows_drug e
WHERE
    (2 * e.start_ordinal) - e.overall_ord = 0
;
DROP TABLE IF EXISTS etl_dataset.tmp_drugera_ends_drug;
CREATE TABLE etl_dataset.tmp_drugera_ends_drug
AS SELECT
...
FROM
    etl_dataset.tmp_finaltarget_drug ft
JOIN
    etl_dataset.tmp_enddates_drug e
...
DROP TABLE IF EXISTS etl_dataset.cdm_drug_era;
...
-- -------------------------------------------------------------------
INSERT INTO etl_dataset.cdm_drug_era
SELECT
...
FROM
    etl_dataset.tmp_drugera_ends_drug
GROUP BY
    person_id,
    drug_era_end_date,
    ingredient_concept_id
