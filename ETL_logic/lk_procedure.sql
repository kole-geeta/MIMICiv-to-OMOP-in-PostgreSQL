...
DROP TABLE IF EXISTS etl_dataset.lk_datetimeevents_concept;
DROP TABLE IF EXISTS etl_dataset.lk_proc_event_clean;
DROP TABLE IF EXISTS etl_dataset.lk_datetimeevents_clean;

-- -------------------------------------------------------------------
-- lk_hcpcsevents_clean
-- Rule 1, HCPCS mapping
-- -------------------------------------------------------------------
DROP TABLE IF EXISTS etl_dataset.lk_hcpcsevents_clean;
CREATE TABLE etl_dataset.lk_hcpcsevents_clean AS
 SELECT
...
FROM
    etl_dataset.src_hcpcsevents src
INNER JOIN
    etl_dataset.src_admissions adm
...
-- Rule 2, ICD mapping
-- -------------------------------------------------------------------
DROP TABLE IF EXISTS etl_dataset.lk_procedures_icd_clean;
CREATE TABLE etl_dataset.lk_procedures_icd_clean AS
SELECT
    src.subject_id                              AS subject_id,
    src.hadm_id                                 AS hadm_id,
    adm.dischtime                               AS start_datetime,
    ...
    src.trace_id                        AS trace_id
FROM
    etl_dataset.src_procedures_icd src
INNER JOIN
    etl_dataset.src_admissions adm
        ON src.hadm_id = adm.hadm_id
;
...
-- -------------------------------------------------------------------
DROP TABLE IF EXISTS etl_dataset.lk_proc_d_items_clean;
CREATE TABLE etl_dataset.lk_proc_d_items_clean AS
SELECT
    src.subject_id                      AS subject_id,
    src.hadm_id                         AS hadm_id,
    ...
    src.trace_id                        AS trace_id
FROM
    etl_dataset.src_procedureevents src
WHERE
...
-- -------------------------------------------------------------------
INSERT INTO etl_dataset.lk_proc_d_items_clean
SELECT
    src.subject_id                      AS subject_id,
    src.hadm_id                         AS hadm_id,
    ...
    src.trace_id                        AS trace_id    
FROM
    etl_dataset.src_datetimeevents src -- de
INNER JOIN
    etl_dataset.src_patients pat
...
-- -------------------------------------------------------------------
DROP TABLE IF EXISTS etl_dataset.lk_hcpcs_concept;
CREATE TABLE etl_dataset.lk_hcpcs_concept AS
SELECT
...
-- ICD - Rule 2
-- -------------------------------------------------------------------
DROP TABLE IF EXISTS etl_dataset.lk_icd_proc_concept;
CREATE TABLE etl_dataset.lk_icd_proc_concept AS
SELECT
...
-- -------------------------------------------------------------------
DROP TABLE IF EXISTS etl_dataset.lk_itemid_concept;
CREATE TABLE etl_dataset.lk_itemid_concept AS
SELECT
    ...
    vc2.concept_id                      AS target_concept_id
FROM
    etl_dataset.src_d_items d_items
LEFT JOIN
    voc_dataset.concept vc
...
-- Rule 1, HCPCS
DROP TABLE IF EXISTS etl_dataset.lk_procedure_mapped;
CREATE TABLE etl_dataset.lk_procedure_mapped AS
SELECT
    ...
    src.trace_id                    AS trace_id
FROM
    etl_dataset.lk_hcpcsevents_clean src
LEFT JOIN
    etl_dataset.lk_hcpcs_concept lc
        ON src.hcpcs_cd = lc.source_code
;

-- Rule 2, ICD

INSERT INTO etl_dataset.lk_procedure_mapped
SELECT
    src.subject_id                          AS subject_id, -- to person
    src.hadm_id                             AS hadm_id, -- to visit
    src.start_datetime                      AS start_datetime,
    ...
    src.trace_id                    AS trace_id
FROM
    etl_dataset.lk_procedures_icd_clean src
LEFT JOIN
    etl_dataset.lk_icd_proc_concept lc
...
SELECT
    ...
    src.trace_id                    AS trace_id
FROM
    etl_dataset.lk_proc_d_items_clean src
LEFT JOIN
    etl_dataset.lk_itemid_concept lc
